...
export function TestNavBar() {
    ...
    return <ul>
		<li><Link href={"/test"}><a>Home</a></Link></li>
			{(user) ? <>
				<li><Link href={"/test/profile"}><a>Profile</a></Link></li>
				<li><a onClick={handleLogout}>Logout</a></li>
			</> : <>
				<li><Link href={"/test/signup"}><a> Sign Up </a></Link></li>
				<li><Link href={"/test/login"}><a>Login</a></Link></li>
			</>}
	</ul>
}
...
export function Posting(){
	...
	const onSubmit = async (e) => {
		e.preventDefault();
		console.log(categoryKey.current.value)
		const fetchBody = {
